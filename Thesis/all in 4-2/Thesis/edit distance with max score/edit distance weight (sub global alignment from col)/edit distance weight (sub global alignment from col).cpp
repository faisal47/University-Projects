    ...
    rep(j,sz(arr2)+1)
    {
        ...
        else
        {
            if(arr1[i-1] == arr2[j-1])
                dp[i][j]=max(dp[i][j],match_value+dp[i-1][j-1]);
            else if(arr1[i-1] != arr2[j-1])
                dp[i][j]=max(dp[i][j],mis_match_value+dp[i-1][j-1]);
            dp[i][j]=max(dp[i][j],gap_value+dp[i-1][j]);
            dp[i][j]=max(dp[i][j],gap_value+dp[i][j-1]);
        }
    }
}
